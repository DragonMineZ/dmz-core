...
/**
 * Handles loading, saving, cloning, and cleanup of component data for players.
 * Each player is associated with a {@link ComponentRegistry} containing
 * instances of all components that are marked as auto-applied. These component
 * instances are stored inside the global {@link ComponentHolderRegistry}.
 *
...
public final class ComponentListener {
    ...
    /**
     * Initializes a fresh {@link ComponentRegistry} for the loading player,
     * instantiates all auto-applied components, registers them, and then loads
     * their stored data from the player's persistent NBT.
     */
    @SubscribeEvent
    ...
    /**
     * Gathers all component data associated with the player and writes the
     * resulting NBT into the player's persistent data under the mod's root tag.
     */
    @SubscribeEvent
    public static void onPlayerSave(PlayerEvent.SaveToFile event) {
        if (!(event.getEntity() instanceof ServerPlayer player)) return;

        final ComponentHolderRegistry provider = RegistryManager.registry(ComponentHolderRegistry.class);
    ...
    /**
     * Transfers component data when the player is cloned (death, dimension
     * change). If the old player has stored data, it is copied. Otherwise,
     * a new default component registry is assigned.
     */
    @SubscribeEvent
    ...
    /**
     * Removes the player’s component data from memory on logout to prevent
     * memory growth. Persistent data remains stored in the player file.
     */
    @SubscribeEvent
